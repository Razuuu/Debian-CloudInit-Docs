# Making a Debian 13 VM Template for Proxmox and CloudInit

This is a short guide for setting up a Debian VM template in Proxmox using CloudInit in a scriptable manner.
...
Now log into Proxmox via SSH so that we can download a cloud image for Debian, then resize this image:
...
The next step is to create a basic VM that we'll build upon:
    ...
    #sudo qm create 8001 --name "debian-13-cloudinit-template" --ostype l26 \
        --memory 1024 \
        --agent 1 \
...
## Configuring hardware
    ...
    sudo qm importdisk 8001 debian-13-generic-amd64-resized.qcow2 local-lvm
...
The first command imports that image we downloaded earlier, if your disk storage is not local-lvm (for example local-zfs) then replace it with whatever you wish. Next command attaches the disk to the VM. If your disk storage is not on SSDs (which it should be) omit `,discard=on`. The third command sets the boot order. Fourth adds the cloudinit pseudo-cdrom drive.
...
## Configuring CloudInit
    ...
    sudo qm set 8001 --tags debian-template,13,cloudinit
